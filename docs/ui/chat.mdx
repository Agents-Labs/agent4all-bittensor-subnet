---
title: "Agent4ALL Chat"
description: "Guide to Agent4ALL's chat interface with support for 100+ LLMs, internet search, vision models, and custom database configurations"
icon: "comments"
---
...
Agent4ALL Chat now includes internet search capabilities using Crawl4AI and Tavily. This feature allows you to retrieve up-to-date information during your conversations, enhancing the AI's ability to provide current and relevant information.
...
These new features significantly expand the capabilities of Agent4ALL Chat, allowing for more diverse and informative interactions.

## Custom Database

Agent4ALL Chat supports custom database configurations, allowing you to use PostgreSQL or other databases instead of the default SQLite database. This is particularly useful for production environments or when you need more advanced database features.
...
If no `DATABASE_URL` is provided, Agent4ALL Chat will automatically use SQLite with the following default configuration:
```bash
...
Agent4ALL Chat supports various database backends through SQLAlchemy:
