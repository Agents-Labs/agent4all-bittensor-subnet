#!/usr/bin/env python3
"""
Agent4All Planning Validator
Specialized validator for planning tasks
"""
...
class PlanningValidator(Validator):
    """Specialized validator for planning category"""
    def __init__(self, config=None):
        ...
        logging.info(f"Initialized planning validator")

    async def evaluate_planning_response(self, response, expected_output=None):
        ...
        try:
            ...
            for response in responses:
                if response.dendrite.status_code == 200:
                    score = await self.evaluate_planning_response(response)
                    scores.append(score)
                else:
                    scores.append(0.0)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Agent4All Planning Validator")
    parser.add_argument("--wallet.name", default="agent4all_coldkey", help="Wallet name")
    parser.add_argument("--wallet.hotkey", default="planning_validator_hotkey", help="Wallet hotkey")
