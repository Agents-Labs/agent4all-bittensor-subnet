...
try:
    import agent4all.api.call as call_module
...
try:
    from praisonaiagents import Agent as PraisonAgent, Task as PraisonTask, Agent4ALLAgents
...
class Agent4ALL:
    def __init__(self, agent_file="agents.yaml", framework="", auto=False, init=False, agent_yaml=None, tools=None):
        """
        Initialize the Agent4ALL object with default parameters.
        """
    ...
    def run(self):
        """
        Run the Agent4ALL application.
        """
        self.main()

    def main(self):
        """
        The main function of the Agent4ALL object. It parses the command-line arguments,
        ...
        self.args = args
        invocation_cmd = "agent4all"
        version_string = f"Agent4ALL version {__version__}"
        ...
        if args.command == 'train':
            ...
            if 'init' in sys.argv:
                from agent4all.setup.setup_conda_env import main as setup_conda_main
                setup_conda_main()
    ...
    def parse_args(self):
        """
        Parse the command-line arguments for the Agent4ALL CLI.
        """
        # Define special commands
        special_commands = ['chat', 'code', 'call', 'realtime', 'train', 'ui']
        
        parser = argparse.ArgumentParser(prog="agent4all", description="agent4all command-line interface")
        parser.add_argument("--framework", choices=["crewai", "autogen", "agent4all"], help="Specify the framework")
        ...
        parser.add_argument("--call", action="store_true", help="Start the Agent4ALL Call server")
        ...
        if not args.command and (args.init or args.auto or args.framework):
            if not CREWAI_AVAILABLE and not AUTOGEN_AVAILABLE and not PRAISONAI_AVAILABLE:
                ...
                print("\npip install \"agent4all\\[crewai]\"  # For CrewAI")
                print("pip install \"agent4all\\[autogen]\"  # For AutoGen")
                print("pip install \"agent4all\\[crewai,autogen]\"  # For both frameworks\n")
                print("pip install praisonaiagents # For Agent4ALLAgents\n")  
                sys.exit(1)
    ...
    def create_chainlit_chat_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            chat_ui_path = os.path.join(os.path.dirname(agent4all.__file__), 'ui', 'chat.py')
            chainlit_run([chat_ui_path])
        else:
            print("ERROR: Chat UI is not installed. Please install it with 'pip install \"agent4all[chat]\"' to use the chat UI.")
    ...
    def create_code_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            code_ui_path = os.path.join(os.path.dirname(agent4all.__file__), 'ui', 'code.py')
            chainlit_run([code_ui_path])
        else:
            print("ERROR: Code UI is not installed. Please install it with 'pip install \"agent4all[code]\"' to use the code UI.")
    ...
    def create_chainlit_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            public_folder = os.path.join(os.path.dirname(agent4all.__file__), 'public')
            if not os.path.exists("public"):
                ...
                logging.info("Public folder already exists.")
            chainlit_ui_path = os.path.join(os.path.dirname(agent4all.__file__), 'ui', 'agents.py')
            chainlit_run([chainlit_ui_path])
        else:
            print("ERROR: Chainlit is not installed. Please install it with 'pip install \"agent4all[ui]\"' to use the UI.")
    ...
    def create_realtime_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            if not os.path.exists(os.path.join(root_path, "public")):
                ...
                logging.info("Public folder already exists.")
            realtime_ui_path = os.path.join(os.path.dirname(agent4all.__file__), 'ui', 'realtime.py')
            chainlit_run([realtime_ui_path])
        else:
            print("ERROR: Realtime UI is not installed. Please install it with 'pip install \"agent4all[realtime]\"' to use the realtime UI.")

if __name__ == "__main__":
    praison_ai = Agent4ALL()
    praison_ai.main()
