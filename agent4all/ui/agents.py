...
from praisonaiagents import Agent, Task, Agent4ALLAgents, register_display_callback
...
async def ui_run_praisonai(config, topic, tools_dict):
    ...
    try:
        ...
        await cl.Message(content="Starting Agent4ALL agents execution...", author="System").send()

        # Decide how to process tasks
        if config.get('process') == 'hierarchical':
            prai_agents = Agent4ALLAgents(
        ...
        await cl.Message(content="Agent4ALL agents execution completed.", author="System").send()
        ...
        settings = await cl.ChatSettings(
            [
                ...
                Select(
                    id="Framework",
                    label="Framework",
                    values=["agent4all", "crewai", "autogen"],
                    initial_index=0,
                ),
            ]
        ).send()
...
async def main(message: cl.Message):
    try:
        ...
        error_msg = f"Error running Agent4ALL agents: {str(e)}"
        logger.error(error_msg, exc_info=True)
        await cl.Message(content=error_msg, author="System").send()
