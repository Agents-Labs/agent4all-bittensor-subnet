...
from .inc import Agent4ALLModel
import inspect
...
class AgentsGenerator:
    def __init__(self, agent_file, framework, config_list, log_level=None, agent_callback=None, task_callback=None, agent_yaml=None, tools=None):
        ...
        if framework == "crewai" and not CREWAI_AVAILABLE:
            raise ImportError("CrewAI is not installed. Please install it with 'pip install agent4all[crewai]'")
        elif framework == "autogen" and not AUTOGEN_AVAILABLE:
            raise ImportError("AutoGen is not installed. Please install it with 'pip install agent4all[autogen]'")
        elif framework == "agent4all" and not PRAISONAI_AVAILABLE:
            raise ImportError("Agent4ALL is not installed. Please install it with 'pip install praisonaiagents'")
    ...
    def generate_crew_and_kickoff(self):
        ...
        elif framework == "agent4all":
            if not PRAISONAI_AVAILABLE:
                raise ImportError("Agent4ALL is not installed. Please install it with 'pip install praisonaiagents'")
            if AGENTOPS_AVAILABLE:
                agentops.init(os.environ.get("AGENTOPS_API_KEY"), default_tags=["agent4all"])
    ...
    def _run_praisonai(self, config, topic, tools_dict):
        """
        Run agents using the Agent4ALL framework.
        """
        agents = {}
        tasks = []
        tasks_dict = {}
        ...
        # Create and run the Agent4ALL agents
