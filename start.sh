...
docker pull agent4allengineering/agent4all-bittensor-subnet:${TAG:-latest}
...
start_node() {
    ...
    docker run -d \
        ...
        -e AGENT4ALL_BASE_URL=https://test.protocol-api.agent4all.ai \
        -e API_URL=https://test.protocol-api.agent4all.ai \
        $ENV_VARS \
        -v $(pwd):/app \
        -v $(pwd)/.env:/app/.env \
        ...
        agent4allengineering/agent4all-bittensor-subnet:${TAG:-latest}
}
